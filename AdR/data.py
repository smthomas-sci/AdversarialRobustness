...
#
#
# def create_data_set(data_directory=None, file_names=None, img_dim=4, batch_size=12, label_dict=None):
#     """
...
#         index = tf.strings.split(filename, os.sep)[0:3]
#         print(index)
#         image = tf.image.decode_jpeg(image)
#         image = tf.image.resize(image, [img_dim, img_dim])
#         image = tf.keras.applications.resnet50.preprocess_input(image)
#         label = tf.one_hot(label_dict[index], len(label_dict.keys()))
#         return image, label
#
#     n = len(file_names)
